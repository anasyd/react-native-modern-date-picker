...
// BEGIN: Full calendar-based picker (ported from main package)
...
export type Theme = {
  ...
  // Global radius. When provided, it will be applied to the individual radii
  // below if they are not specified explicitly.
  radius?: number;
  topRadius?: number;
  ...
  // Simple theming API
  preset?: "dark" | "light"; // chooses base color palette when colors not provided
  primary?: string; // main accent (selectedBackground, todayBorder)
  secondary?: string; // secondary surfaces (headerBackgroundColor, surface)
  shadow?: boolean;
...
const DEFAULT_THEME: Theme = {
  ...
  shadow: true,
  topRadius: 20,
  // selectedDateRadius default: circle (handled in render if undefined)
...
// High-contrast black/white palette used when no theme.colors are provided
const BW_COLORS: Required<NonNullable<Theme["colors"]>> = {
  background: "#000000",
  surface: "#000000",
...
// Light palette
const LIGHT_COLORS: Required<NonNullable<Theme["colors"]>> = {
  background: "#FFFFFF",
  surface: "#FFFFFF",
...
// Export preset palettes for users who want to build custom themes
...
const ModernDatePicker: React.FC<ModernDatePickerProps> = ({
  ...
  // Fixed body height to avoid jumping between 5/6 week months and mode switches
  const BODY_HEIGHT = 360;
  const THEME = React.useMemo(() => {
    // Choose base palette from preset if provided, otherwise default dark when colors are missing
    ...
    // Apply primary/secondary overrides to colors
    if (merged.primary) {
  ...
  useEffect(() => {
    if (open) {
      // Ensure mounted to play enter animation
      ...
      {matrix.map((row: { date: Date; inMonth: boolean }[], r: number) => (
        <View key={r} style={styles.weekRow}>
          {row.map((cell: { date: Date; inMonth: boolean }, c: number) => {
            ...
            // - If selectedDateRadius is undefined -> circle
            // - If selectedDateRadius is 0 -> square
            // - If provided > 0 -> that radius
            const selectedRadius = THEME.selectedDateRadius;
            const computedRadius = selected
              ? selectedRadius === undefined
                ? 9999 // circle by default
                : selectedRadius // square when 0; rounded when > 0
              : 8; // unselected corners
      ...
      <Pressable style={styles.backdrop} onPress={onClose}>
        ...
        >
          <BlurView
            ...
            tint="dark"
            // Android needs experimentalBlurMethod to enable software blur
            // This prop is ignored on other platforms
            experimentalBlurMethod={
              Platform.OS === "android" ? ("dimezisBlurView" as any) : undefined
            }
...
const styles = StyleSheet.create({
  ...
  backdropTint: { flex: 1, backgroundColor: "rgba(0,0,0,0.35)" },
  sheet: { position: "absolute", left: 0, right: 0, bottom: 0 },
  // Clip children so topRadius stays visible even while animating
  // (prevents child backgrounds from showing squared corners)
  // Applied conditionally via style spread
  ...
  dayCell: { width: f(100 / 7) + "%", alignItems: "center" } as any,
  dayPill: {
    width: 36,
    height: 36,
    alignItems: "center",
...
// END: Full calendar-based picker
